...
- 🟡 **IN PROGRESS** — PIT4 Temporary Wage Subsidy Scheme (TWSS) Reconciliation Description (`content/PIT4/screens/twss_reconciliation_csv_description.html`) — started 2026-07-09 (PIT4 only, no PIT3 equivalent document exists). Pipeline run complete; diagnosed the same bug pattern as twss_operational_phase: (1) Column Descriptions and Latest Version History headings have no table underneath them — both tables misplaced further down the document after Audience/Document context; (2) main data-dictionary table (Employer Name...ARNWP) has phantom empty columns throughout; (3) table is genuinely split across a PDF page break, with a later fragment's first data row ('Subsidy Paid') wrongly promoted into a `<thead>` row. **No fix has been written or applied yet** — `fix_twss_reconciliation()` does not exist in `tools/run_doc_fixes.py`, and no registry entry exists in `tools/doc_fixes_registry.json`. The current on-disk HTML is raw pipeline output and is NOT ready for review. Next step: read the source PDF directly (4 pages) to establish ground truth for the full main table content before writing any fix — see `SESSION_LOG.md` 2026-07-09 entry for full detail. **Resuming 20/07/2026.**
